...
if (!document.getElementById("EmbedSocialLightboxCSS")) {
    ...
    getEmbedData: function (e, t) {
        ...
        m.setAttribute("id", "embedIFrame_" + e + Math.random().toString(36).substring(7)),
        m.style.width = "0px", m.style.height = "0px",
        m.style.maxHeight = "100%",
        m.style.maxWidth = "100%",
        m.style.minHeight = "100%",
        m.style.minWidth = "100%",
        m.style.border = "0",
        m.setAttribute("class", "embedsocial-in-album-iframe"), m.setAttribute("scrolling", "yes"),
        t.appendChild(m),
    ...
    initResizeFrame: function () { 
        ...
        ? iFrameResize({ messageCallback: function (e) { EMBEDSOCIALINSTAGRAM.createLightBox(e.message) } }, ".embedsocial-in-album-iframe")
    ...
    createLightBox: function (e) {
        if (document.getElementById("embedSocialLightboxDiv") && document.getElementById("embedSocialLightboxDiv").getAttribute("data-ref") == e.albumRef && document.getElementById("embedSocialLightboxDiv").getAttribute("data-num") == e.albumNum && document.getElementById("embedSocialLightboxDiv").getAttribute("data-captions") == e.showCaptions)
            for (var t = 0; t < e.albumImages.length; t++)
                1 == e.albumImages[t].click && EMBEDSOCIALINSTAGRAM.openLightBox(e.albumImages[t].id);
        else {
            ...
            } 1 == e.showCaptions ? embedLightBox.run(".embedSocialLightboxDiv", { captions: function (e) { return e.getElementsByTagName("img")[0].alt }, filterlightem: new RegExp("((.+.(gif|jpe?g|png|webp))|size=l$)", "i") }) : embedLightBox.run(".embedSocialLightboxDiv", { filterlightem: new RegExp("((.+.(gif|jpe?g|png|webp))|size=l$)", "i") });
            for (t = 0; t < e.albumImages.length; t++)
            1 == e.albumImages[t].click && EMBEDSOCIALINSTAGRAM.openLightBox(e.albumImages[t].id)
        }
    },
...
var embedsocialInstagramAlbums = document.getElementsByClassName("embedsocial-instagram"), embedsocialInstagramAlbumsRef = [];
